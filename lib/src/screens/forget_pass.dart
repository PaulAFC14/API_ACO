...
class _ForgetPassScreenState extends State<ForgetPassScreen> {
        ...
        child: ListView(
          children: <Widget>[
            Container(
              ...
              child: Image.asset('assets/img/forgot_guy.png',),
            ),
            ...
            Form(
              ...
              child: Column(
                children: [
                  InputText().generateInputForm(
                    context, controller: mailController, hide: false, 
                    hint: 'Email', width: 0, color: mailColor
                  ),
                  SizedBox(height: 5),
                  InputText().EmptyLabel(context, label: 'Missing email', active: empty)
                ],
              ),
            ),
            SizedBox(height: 50),
            RoundedButton(
              backgroundColor: Theme.of(context).primaryColor, color: Colors.white, txt: 'Submit', 
              height: 60, width: 0
  ...
  Future<void> send() async{
    var request = http.MultipartRequest('POST', Uri.parse('https://crud.jonathansoto.mx/api/forgot-password'));
    ...
    if (response.statusCode == 200) {
      ...
      MyDialog(context).SuccessfullySent();
    }
    else {
